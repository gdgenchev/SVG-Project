...
void getInfo(std::fstream& filestr) {
    figc.defaultConstructor(); // new collection for every opened file
    Line* line;
    Rectangle* rect;
    Circle* circ;
    std::string word;
    ...
    while (true) {
        ...
        if (!strcmp(c, "open")) {
            ...
            if (filestr.is_open()) {
                ...
                if (!strstr(filePath, ".svg")) {
                    ...
                    createFile << "<?xml version=\"1.0\" standalone=\"no\"?>\n";
                    createFile << "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" \n";
                    createFile << "\"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">\n";
                    createFile << "<svg width=\"40cm\" height=\"20cm\" viewBox=\"0 0 1200 700\"\n";
                    createFile << "xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\">\n";
                    createFile << "<desc>Created new file</desc>\n";
                }
            }
        }
        ///*Print
        ...
        if (!strcmp(c, "saveas")) {
            if (opened) {
                ...
                std::ofstream newFile(filePath1_fixed, std::ios::ate);
